...
- [x] 进度接口（summary / wrong-words / rebuild）—— `routers/progress.py`，33 个测试
  - [x] `summary` —— 连续天数、今日答题量、盒子分布、到期待复习
    - [x] ⚠️ **按客户端时区切分"今日"**（`tzOffsetMinutes`）——
          按 UTC 算会把东八区早上 8 点前的学习记到前一天
    - [x] 连续天数："今天没学不算断"，从今天或昨天起往回数
  - [x] `wrong-words` —— 错题本，从 `answer_events` 聚合
    - [x] `recentInputs` 记录最近几次错误输入（事件溯源"免费送"的能力）
    - [x] 按模式筛选 + 分页
  - [x] `rebuild` —— ⭐ **从事件全量重算进度**，兑现 ADR-002 的承诺
    - [x] 顺带清理孤儿进度行（有进度但无事件）
    - [x] 端到端验证：删掉 7 行进度 → 从 10 条事件重建 → **结果完全一致**

> ## ✅ M2 完成
>
> 11 个接口全部可用，**443 个测试**。
>
> 核心算法全是纯函数：Leitner、事件回放、听写判定、干扰项生成。
> 混合式事件溯源（ADR-002）的承诺已被端到端验证 —— 进度表删了能从事件重建。

---

### M2.5 测试模式与标记（用户在 M3 设计访谈中新增，[ADR-013](./08-decisions.md)）

- [x] 数据模型
  - [x] `answer_events` 加 `is_test` / `test_session_id` / `corrects_event_id`
  - [x] 新表 `test_sessions`（一次测试的元信息）
  - [x] 新表 `word_stars`（星标；**单独存**，因为不是事件的推论）
  - [x] 迁移往返测试通过，**且在有 10 行数据的库上验证不丢数据**
- [x] 回放逻辑
  - [x] 测试事件回放时跳过
  - [x] 更正事件把被指向的事件视为答对（追加更正，不改写历史）
  - [x] `replay_incremental` 拒绝这两类事件（早失败好过静默算错）
- [x] 接口（新增 7 个，共 **18 个**）
  - [x] `POST /api/practice/test` —— 开始测试（范围：已学过/全库/话题/盒子）
  - [x] `GET /api/progress/tests` + `/{id}` —— 测试历史与详情
  - [x] `POST /api/practice/correct` —— 判我对（4 道限制防滥用）
  - [x] `PUT/DELETE /api/words/{id}/star` + `GET /api/words/starred` —— 星标
- [x] 35 个测试，端到端验证三条不变式

> **测试模式的核心不变式**：测试答题绝不影响 Leitner 进度。
> 实测：连对 4 次到 Box 5 → 测试 3 题全错（0 分）→ 该词仍是 Box 5、wrongCount=0。
...
- [ ] Tailwind + shadcn/ui 接入
- [ ] API 层封装 —— 建议从 `/openapi.json` 生成 TS 类型
- [ ] 路由 + 登录页
- [ ] **导航栏**：`[听写] [阅读] [进度]` + 右上角用户信息（点开进用户页）
- [ ] **听写页 / 阅读页** 各含 3 个 Tab：`每日任务 | 自由练习 | 测试模式`
  - [ ] 准备页显示题量，点"开始"才进答题
  - [ ] 测试模式准备页多一组范围选择（已学过/全库/话题/盒子）
- [ ] **答题页**（两种模式共用骨架）
  - [ ] 答对自动跳下一题，答错停下看高亮
  - [ ] 左上角常驻"上一词"卡片（词 + 释义 + 你的输入）
  - [ ] 键位：`Enter` 提交/继续 · `Tab` 重播 · `Esc` 回看上一词
  - [ ] 回看面板里 `S` 星标 · `K` 判我对
  - [ ] 听写：音频可开关，默认自动播放；`audio_url` 为空时降级浏览器 TTS
  - [ ] 阅读：`1234` 选项 · `空格` 不知道
  - [ ] ⚠️ 阅读提交回传**选中选项的文本**，不是 index
- [ ] 结果页（本轮正确率、盒子变化；测试模式显示成绩）
- [ ] 进度总览页（盒子分布、连续天数）
  - [ ] ⚠️ 调 `summary` 必须带 `tzOffsetMinutes`，否则东八区早上 8 点前的学习会记到前一天
- [ ] 错题本页（含"最近怎么拼错的"）
- [ ] 星标页 / 测试记录页
