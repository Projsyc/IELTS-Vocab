...
- [ ] 词库数据源调研（ECDICT 或其他开源词典）
- [ ] 词库导入脚本 `scripts/seed_words.py` —— 4000 词
- [ ] LLM 批量打话题标签脚本 `scripts/tag_topics.py`
  - [ ] 抽样人工校验标签质量
- [ ] TTS 音频批量生成脚本 `scripts/gen_audio.py`
...
| 风险 | 状态 | 应对 |
|------|------|------|
| 版权（刘洪波词汇真经） | 🟡 已识别 | 只用选词范围，释义走开源词典。见 [01 §7.1](./01-product-spec.md) |
| LLM 话题标签质量 | 🟡 待验证 | M1 抽样校验 |
| TTS 服务条款 | 🟡 待确认 | 本地使用风险低，部署前复核 |
| 词库数据源找不到合适的 | 🔴 未验证 | **M1 第一件事就是调研，卡住要尽早暴露** |
