...
## 2026-07-25 (10) 测试模式与标记 —— M3 访谈中新增的需求

**起因**

做 M3 前端设计访谈时，用户在回答"点导航栏之后直接答题还是先进准备页"时，
没有选选项，而是提了个新需求：

> 新增测试模式 Tab —— 学习模式错词后可能会有重复，
> 测试模式就是错了就是错了，然后记录下来

后续访谈又加了"回看上一词"和"标记（重点关注 / 其实我会）"。
这三条里有两条要改数据模型，所以**先补后端再做前端**。

**做了什么**

| 模块 | 内容 | 测试 |
|------|------|------|
| 数据模型 | `answer_events` 加 3 列 + `test_sessions` + `word_stars` | — |
| `services/replay.py` | 跳过测试事件、应用更正 | +13 |
| `services/testing.py` | 测试模式、判我对、星标 | — |
| `routers/testing.py` | 新增 7 个接口（共 18 个） | 35 |

测试 443 → **491**。

**三条不变式（都有测试固化）**

1. `user_progress` 能从 `answer_events` 完整重算（ADR-002，之前就有）
2. **测试模式的答题绝不影响 Leitner 进度**（新）
3. **星标不在 `user_progress` 里**，重建不该抹掉它（新）

第 3 条正是 ADR-002 那条铁律的正确用法：遇到"想往 progress 里塞点东西"时，
先问它是不是事件的推论。星标是用户主动打的书签，不可能从答题算出来，
塞进去就会被 `rebuild` 清掉。已写 `test_stars_survive_rebuild` 固化。

**"判我对"为什么是追加更正而不是改写**

事件表只 INSERT 是 ADR-002 的第一条。改写历史会让"重放得到同样结果"
依赖于"历史没被人改过"，那就不是事实来源了。

所以追加一条 `corrects_event_id` 指向原事件的更正，回放时应用：

```
第一趟：收集被更正的事件 id
第二趟：排除更正事件本身与测试事件，回放时把被更正的视为答对
```

好处：原始错误仍可查（能看到"我当时拼成了 accomodate"），更正本身可审计。

四道限制防滥用：只能改自己的、原本必须答错、**测试模式不能用**、
**编辑距离 ≤ 2**（阅读模式也不给用，选错就是选错）。

**踩坑一：autogenerate 不知道表里有数据（BUG-009）**

生成的迁移是 `add_column('is_test', Boolean, nullable=False)` —— 没有 `server_default`。
表里**已有 10 行数据**，这条一定会失败。

模型里的 `default=False` 是 **Python 层默认值**（插入时由 SQLAlchemy 填），
不是数据库的 DEFAULT 约束，所以 autogenerate 不会生成 `server_default`。

同一个迁移还有 ENUM 重复创建（BUG-004 同款）和 `drop_constraint(None, ...)` 两个老问题。
三处都手工改了，并在**有数据的库**上跑往返测试，验证 10 行数据全程没丢。

**踩坑二：加了字段却漏了两处查询（BUG-010）—— 静默算错**

模型和 `replay()` 都改好了，单测全过。但接口测试挂两条：

```
判我对后期望 Box 5，实际 Box 2
```

根因：**从数据库构造 `AnswerRecord` 的两处查询没加新字段**。
新字段有默认值，所以不报错 —— 更正事件被当成"又答对一次"（Box 1→2），
测试答错被当成真实答错。

Box 2 这个数字正好印证：4 次对 + 1 次错(Box 1) + 把更正当成一次对(Box 2)。

> 教训：**带默认值的新字段是双刃剑** —— 让旧代码不报错，
> 也让"忘了传"变成静默错误。加字段后必须 `grep` 所有构造点。
>
> 还有一点值得记：**单测全过是因为单测手工构造 `AnswerRecord`（字段齐全）**，
> 真正抓到问题的是走数据库的接口测试。两层测试都需要。

**端到端验证**

```
连对 4 次 → Box 5
测试 3 题全答错 → 成绩 0 分，但该词仍 Box 5、wrongCount=0     ✅
typo → Box 1 → 判我对 → 恢复 Box 5                          ✅
星标 + 全量重建 → 盒子分布不变、星标仍在                       ✅
```

**下次从哪继续 —— M3 前端**

设计已在访谈中确认（见 [05-roadmap](./05-roadmap.md) M3 章节）：

- 导航栏 `[听写] [阅读] [进度]` + 右上角用户信息
- 听写/阅读页各含 3 个 Tab：每日任务 / 自由练习 / 测试模式
- 答对自动跳、答错停下看高亮；左上角常驻"上一词"卡片
- 键位：`Enter` 提交/继续 · `Tab` 重播 · `Esc` 回看 · 回看面板 `S` 星标 `K` 判我对
- 技术：React Router + TanStack Query + 从 `/openapi.json` 生成 TS 类型
